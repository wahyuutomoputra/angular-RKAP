...
import {Menu, RkapRelease, KategoriAktif} from '../rkap-release.model';
...
@Component({
  selector: 'app-add-rkap-release',
  templateUrl: './add-rkap-release.component.html',
  providers: []
})
// export class AddRkapReleaseComponent implements OnInit {
...
// }
export class AddRkapReleaseComponent implements OnInit, AfterViewInit, AfterContentInit {
  @Input() isEdit;
  @Input() isDetail;
  @Input() editItem;
  @Input() addVisible;
  ...
  constructor(private rkapreleaseService: RkapReleaseService) {
    this.simpleProducts = rkapreleaseService.getSimpleProducts();
    this.rkapreleaseService.getRkap().subscribe(resp=>{
      this.daftarRkap = resp.d.list;
    })
  ...
  ngOnInit() {
   if (this.isEdit || this.isDetail) {
     ...
     this.rkapreleaseService.getById(this.editItem).subscribe(respRole => {
       console.log(respRole);
       console.log(this.editItem);
       const nilai: string = respRole.d.releaseid;
       this.role = {
         releaseid: respRole.d.releaseid,
         released_date: respRole.d.released_date,
         //urutan: parseInt(nilai),
         released_by: respRole.d.released_by,
         nilai_release: respRole.d.nilai_release,
         nilai_rkap: respRole.d.nilai_rkap,
         nilai_sisa: respRole.d.nilai_sisa,
 ...
 checkBoxToggled(e) {
  ...
  onSaveConf() {
    ...
    const nilai: number = this.role.releaseid;
    this.role.releaseid = nilai;
    let success = false;
    if (!this.isEdit) {
      this.role.releaseid = null;
      this.rkapreleaseService.save(this.role).subscribe(resp => {
        console.log(resp);
        success = true;
        this.menuTree.forEach(menuItem => {
          if (menuItem.read || menuItem.write) {
            ...
            this.rkapreleaseService.saveRoleAuth({
              read: read,
              write: write,
        ...
        if (success) {
          ...
          notify({message: 'Rkap Release berhasil disimpan', position: {my: 'center top', at: 'center top'}},
           'success', 3000);
          this.hide();
        } else {
      ...
      this.rkapreleaseService.update(this.role).subscribe(resp => {
       ...
       success = true;
        ...
        this.menuTree.forEach(menuItem => {
          if (menuItem.read || menuItem.write) {
            ...
            if (typeof menuItem.authId === 'undefined') {
             ...
             this.rkapreleaseService.updateRoleAuth({
            ...
            if (typeof menuItem.authId !== 'undefined') {
              this.rkapreleaseService.deleteRoleAuth(menuItem.authId).subscribe(() => { }, () => {
                success = false;
              });
            }
          }
        ...
        if (success) {
          this.options.message = 'Rkap Release updated';
          notify(this.options, 'success', 3000);
          this.hide();
        } else {
