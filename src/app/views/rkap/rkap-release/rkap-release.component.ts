...
import {Search} from './rkap-release.model';
...
@Component({
  selector: 'app-rkap-release',
  templateUrl: './rkap-release.component.html',
  styleUrls: ['./rkap-release.component.scss'],
  providers: [RkapReleaseService]
})
export class RkapReleaseComponent implements AfterViewInit {
  ...
  constructor(
    ...
    @Inject(RkapReleaseService) private rkapreleaseService: RkapReleaseService
  ) {
    this.search = {
      releaseid: null,
      released_date: '',
    ...
    //this.rkapreleaseService.getAll()
    //.subscribe(resp => {
    //  console.log(resp);
  ...
  pagination(){
    ...
    this.rkapreleaseService.getLimit(this.offset,this.limitVal)
    .subscribe(resp => {
      console.log(resp);
      this.gridDataSource = resp.d.list;
  ...
  refresh() {
    this.rkapreleaseService.getAll().subscribe(resp => {
      this.gridDataSource = resp.d.list;
    }, err => {
      console.log(err);
    })
  ...
  getRoleData() {
    ...
    this.rkapreleaseService.getAll().subscribe(resp => {
      this.rkapreleaseService.getAllRoleAuth().subscribe(respAuth => {
  ...
  searching() {
    this.rkapreleaseService.getByName(this.search.released_by).subscribe(resp => {
      this.rkapreleaseService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
      ...
      if (this.search.released_by !== '') {
        this.gridDataSource = resp.filter(role => role.roleName === this.search.released_by).filter(data => data.activationCode === 'Y');
      } else {
        this.gridDataSource = resp.filter(data => data.activationCode === 'Y');
      }
    });
  ...
  advSearch() {
    this.rkapreleaseService.getByData(this.search).subscribe(resp => {
      this.rkapreleaseService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  onDeleteConf() {
    this.rkapreleaseService.getById(this.target).subscribe(role => {
      this.rkapreleaseService.delete(role.d).subscribe(resp => {
        // this.getRoleData();
        notify({
          closeOnClick: true,
          displayTime: 3000,
