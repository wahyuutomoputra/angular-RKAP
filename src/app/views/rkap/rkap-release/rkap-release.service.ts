...
import { RkapRelease, KategoriAktif } from './rkap-release.model';
...
//   providedIn: 'root'
// })
// export class RkapReleaseService {
...
@Injectable()
export class RkapReleaseService {
  private resourceUrlRole = this.a.SERVER_URL + '/system/RKAPReleased';
  ...
  getRkap(): Observable<any>{
    return this.http.get(this.a.SERVER_URL + '/system/RKAP/table')
  }


  getById(id: any): Observable<any> {
    const token = localStorage.getItem('token');
    return this.http.get(this.resourceUrlRole + '/retrieve?token=' + token + '&releaseid=' + id)
  }
  ...
  save(data: RkapRelease): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
  ...
  update(data: RkapRelease): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
    const today = new Date().toISOString().slice(0, 10);
    console.log(data);
  ...
  delete(data: any): Observable<any> {
    ...
    const token = localStorage.getItem('token');
    return this.http.get(this.resourceUrlRole + '/delete?token=' + token + '&releaseid=' + data.releaseid)
  }
