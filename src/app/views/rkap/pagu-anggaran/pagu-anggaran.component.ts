...
import {Search} from './pagu-anggaran.model';
...
@Component({
  selector: 'app-pagu-anggaran',
  templateUrl: './pagu-anggaran.component.html',
  styleUrls: ['./pagu-anggaran.component.scss'],
  providers: [PaguAnggaranService]
})
// export class PaguAnggaranComponent implements OnInit {
...
// }
export class PaguAnggaranComponent implements AfterViewInit {
  ...
  constructor(
    ...
    @Inject(PaguAnggaranService) private paguanggaranService: PaguAnggaranService
  ) {
    this.search = {
      pagu_id: null,
      deptid: null,
    ...
    //this.paguAnggaranService.getAll()
    //.subscribe(resp => {
    //  console.log(resp);
  ...
  pagination(){
    ...
    this.paguanggaranService.getLimit(this.offset,this.limitVal)
    .subscribe(resp => {
      console.log(resp);
      this.gridDataSource = resp.d.list;
  ...
  refresh() {
    this.paguanggaranService.getAll().subscribe(resp => {
      this.gridDataSource = resp.d.list;
    }, err => {
      console.log(err);
    })
  ...
  getRoleData() {
    ...
    this.paguanggaranService.getAll().subscribe(resp => {
      this.paguanggaranService.getAllRoleAuth().subscribe(respAuth => {
  ...
  addNewButton() {
    ...
    const $customButton = $('<div id="addNewRole">').dxButton({
      icon: 'add',
      text: 'Tambah Pagu Anggaran Baru',
      onClick: function () {
        ini.isAdd = true;
  ...
  searching() {
    this.paguanggaranService.getByName(this.search.nilai_pagu).subscribe(resp => {
      this.paguanggaranService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  advSearch() {
    this.paguanggaranService.getByData(this.search).subscribe(resp => {
      this.paguanggaranService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  onDeleteConf() {
    this.paguanggaranService.getById(this.target).subscribe(role => {
      this.paguanggaranService.delete(role.d).subscribe(resp => {
        // this.getRoleData();
        notify({
          closeOnClick: true,
          displayTime: 3000,
