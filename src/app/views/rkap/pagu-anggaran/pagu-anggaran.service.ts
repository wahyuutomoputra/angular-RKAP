...
import { PaguAnggaran, KategoriAktif } from './pagu-anggaran.model';
...
//   providedIn: 'root'
// })
// export class PaguAnggaranService {
...
@Injectable()
export class PaguAnggaranService {
  private resourceUrlRole = this.a.SERVER_URL + '/system/PaguAnggaran';
  ...
  getById(id: any): Observable<any> {
    ...
    return this.http.get(this.resourceUrlRole + '/retrieve?token=' + token + '&pagu_id=' + id)
  }
  ...
  save(data: PaguAnggaran): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
  ...
  update(data: PaguAnggaran): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
    const today = new Date().toISOString().slice(0, 10);
    console.log(data);
  ...
  delete(data: any): Observable<any> {
    ...
    const token = localStorage.getItem('token');
    return this.http.get(this.resourceUrlRole + '/delete?token=' + token + '&pagu_id=' + data.pagu_id)
  }
