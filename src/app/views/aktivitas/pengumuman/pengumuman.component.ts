...
import {Search} from './pengumuman.model';
...
export class PengumumanComponent implements AfterViewInit {
  ...
  constructor(
    ...
    @Inject(PengumumanService) private pengumumanService: PengumumanService
  ) {
    this.search = {
      pid: null,
      isi: '',
  ...
  pagination(){
    ...
    this.pengumumanService.getLimit(this.offset,this.limitVal)
    .subscribe(resp => {
      console.log(resp);
      this.gridDataSource = resp.d.list;
  ...
  refresh() {
    this.pengumumanService.getAll().subscribe(resp => {
      this.gridDataSource = resp.d.list;
    }, err => {
      console.log(err);
    })
  ...
  getRoleData() {
    ...
    this.pengumumanService.getAll().subscribe(resp => {
      this.pengumumanService.getAllRoleAuth().subscribe(respAuth => {
  ...
  searching() {
    this.pengumumanService.getByName(this.search.isi).subscribe(resp => {
      this.pengumumanService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
      ...
      if (this.search.isi !== '') {
        this.gridDataSource = resp.filter(role => role.roleName === this.search.isi).filter(data => data.activationCode === 'Y');
      } else {
        this.gridDataSource = resp.filter(data => data.activationCode === 'Y');
      }
    });
  ...
  advSearch() {
    this.pengumumanService.getByData(this.search).subscribe(resp => {
      this.pengumumanService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  onDeleteConf() {
    this.pengumumanService.getById(this.target).subscribe(role => {
      this.pengumumanService.delete(role.d).subscribe(resp => {
        // this.getRoleData();
        notify({
          closeOnClick: true,
          displayTime: 3000,
