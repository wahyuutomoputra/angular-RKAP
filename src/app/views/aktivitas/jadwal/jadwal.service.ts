...
import { Jadwal, KategoriAktif } from './jadwal.model';

const simpleProducts: string[] = [ 'Y', 'N' ];

@Injectable()
export class JadwalService {
  private resourceUrlRole = this.a.SERVER_URL + '/system/Jadwal';
  ...
  getById(id: any): Observable<any> {
    ...
    return this.http.get(this.resourceUrlRole + '/retrieve?token=' + token + '&jadwalid=' + id)
  }
  ...
  save(data: Jadwal): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
  ...
  update(data: Jadwal): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
    const today = new Date().toISOString().slice(0, 10);
    console.log(data);
  ...
  delete(data: any): Observable<any> {
    ...
    const token = localStorage.getItem('token');
    return this.http.get(this.resourceUrlRole + '/delete?token=' + token + '&jadwalid=' + data.jadwalid)
  }

  getByName(roleName: any): Observable<any> {
    return this.http.get(this.resourceUrlRole + '/retrieve?visi=' + roleName)
