...
import {Menu, IndikatorInput, KategoriAktif} from '../indikator-input.model';
...
@Component({
  selector: 'app-add-indikator-input',
  templateUrl: './add-indikator-input.component.html',
  providers: []
})
// export class AddIndikatorInputComponent implements OnInit {
...
// }
export class AddIndikatorInputComponent implements OnInit, AfterViewInit, AfterContentInit {
  @Input() isEdit;
  @Input() isDetail;
  @Input() editItem;
  @Input() addVisible;
  ...
  constructor(private indikatorinputService: IndikatorInputService) {
    this.simpleProducts = indikatorinputService.getSimpleProducts();
  ...
  ngOnInit() {
   if (this.isEdit || this.isDetail) {
     ...
     this.indikatorinputService.getById(this.editItem).subscribe(respRole => {
       console.log(respRole);
       console.log(this.editItem);
       const nilai: string = respRole.d.indid;
       this.role = {
         indid: respRole.d.indid,
 ...
 checkBoxToggled(e) {
  ...
  onSaveConf() {
    ...
    const nilai: number = this.role.indid;
    this.role.indid = nilai;
    let success = false;
    if (!this.isEdit) {
      this.role.indid = null;
      this.indikatorinputService.save(this.role).subscribe(resp => {
        console.log(resp);
        success = true;
        this.menuTree.forEach(menuItem => {
          if (menuItem.read || menuItem.write) {
            ...
            this.indikatorinputService.saveRoleAuth({
              read: read,
              write: write,
        ...
        if (success) {
          ...
          notify({message: 'Indikator Input berhasil disimpan', position: {my: 'center top', at: 'center top'}},
           'success', 3000);
          this.hide();
        } else {
      ...
      this.indikatorinputService.update(this.role).subscribe(resp => {
       ...
       success = true;
        ...
        this.menuTree.forEach(menuItem => {
          if (menuItem.read || menuItem.write) {
            ...
            if (typeof menuItem.authId === 'undefined') {
             ...
             this.indikatorinputService.updateRoleAuth({
            ...
            if (typeof menuItem.authId !== 'undefined') {
              this.indikatorinputService.deleteRoleAuth(menuItem.authId).subscribe(() => { }, () => {
                success = false;
              });
            }
          }
        ...
        if (success) {
          this.options.message = 'Indikator Input updated';
          notify(this.options, 'success', 3000);
          this.hide();
        } else {
