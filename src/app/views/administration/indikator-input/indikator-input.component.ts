...
import {Search} from './indikator-input.model';
...
@Component({
  selector: 'app-indikator-input',
  templateUrl: './indikator-input.component.html',
  styleUrls: ['./indikator-input.component.scss'],
  providers: [IndikatorInputService]
})
// export class IndikatorInputComponent implements OnInit {
...
// }
export class IndikatorInputComponent implements AfterViewInit {
  ...
  constructor(
    ...
    @Inject(IndikatorInputService) private indikatorinputService: IndikatorInputService
  ) {
    this.search = {
      indid: null,
      rkapid: null,
    ...
    //this.indikatorinputService.getAll()
    //.subscribe(resp => {
    //  console.log(resp);
  ...
  pagination(){
    ...
    this.indikatorinputService.getLimit(this.offset,this.limitVal)
    .subscribe(resp => {
      console.log(resp);
      this.gridDataSource = resp.d.list;
  ...
  refresh() {
    this.indikatorinputService.getAll().subscribe(resp => {
      this.gridDataSource = resp.d.list;
    }, err => {
      console.log(err);
    })
  ...
  getRoleData() {
    ...
    this.indikatorinputService.getAll().subscribe(resp => {
      this.indikatorinputService.getAllRoleAuth().subscribe(respAuth => {
  ...
  addNewButton() {
    ...
    const $customButton = $('<div id="addNewRole">').dxButton({
      icon: 'add',
      text: 'Tambah Indikator Input Baru',
      onClick: function () {
        ini.isAdd = true;
  ...
  searching() {
    this.indikatorinputService.getByName(this.search.indikator).subscribe(resp => {
      this.indikatorinputService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  advSearch() {
    this.indikatorinputService.getByData(this.search).subscribe(resp => {
      this.indikatorinputService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  onDeleteConf() {
    this.indikatorinputService.getById(this.target).subscribe(role => {
      this.indikatorinputService.delete(role.d).subscribe(resp => {
        // this.getRoleData();
        notify({
          closeOnClick: true,
          displayTime: 3000,
