...
import { IndikatorInput, KategoriAktif } from './indikator-input.model';
...
//   providedIn: 'root'
// })
// export class IndikatorInputService {

//   constructor() { }
// }
@Injectable()
export class IndikatorInputService {
  private resourceUrlRole = this.a.SERVER_URL + '/system/IndikatorInput';
  ...
  getById(id: any): Observable<any> {
    ...
    return this.http.get(this.resourceUrlRole + '/retrieve?token=' + token + '&indid=' + id)
  }
  ...
  save(data: IndikatorInput): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
  ...
  update(data: IndikatorInput): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
    const today = new Date().toISOString().slice(0, 10);
    console.log(data);
  ...
  delete(data: any): Observable<any> {
    ...
    const token = localStorage.getItem('token');
    return this.http.get(this.resourceUrlRole + '/delete?token=' + token + 'indid=' + data.indid)
  }
