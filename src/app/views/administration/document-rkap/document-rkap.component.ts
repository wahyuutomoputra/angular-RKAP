...
import {Search} from './document-rkap.model';
...
@Component({
  selector: 'app-document-rkap',
  templateUrl: './document-rkap.component.html',
  styleUrls: ['./document-rkap.component.scss'],
  providers: [DocumentRkapService]
})
// export class DocumentRKAPComponent implements OnInit {
...
// }
export class DocumentRKAPComponent implements AfterViewInit {
  ...
  constructor(
    ...
    @Inject(DocumentRkapService) private documentrkapService: DocumentRkapService
  ) {
    this.search = {
      docid: null,
      rkapid: null,
    ...
    //this.documentrkapService.getAll()
    //.subscribe(resp => {
    //  console.log(resp);
  ...
  pagination(){
    ...
    this.documentrkapService.getLimit(this.offset,this.limitVal)
    .subscribe(resp => {
      console.log(resp);
      this.gridDataSource = resp.d.list;
  ...
  refresh() {
    this.documentrkapService.getAll().subscribe(resp => {
      this.gridDataSource = resp.d.list;
    }, err => {
      console.log(err);
    })
  ...
  getRoleData() {
    ...
    this.documentrkapService.getAll().subscribe(resp => {
      this.documentrkapService.getAllRoleAuth().subscribe(respAuth => {
  ...
  searching() {
    this.documentrkapService.getByName(this.search.nama_dokumen).subscribe(resp => {
      this.documentrkapService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
      ...
      if (this.search.nama_dokumen !== '') {
        this.gridDataSource = resp.filter(role => role.roleName === this.search.nama_dokumen).filter(data => data.activationCode === 'Y');
      } else {
        this.gridDataSource = resp.filter(data => data.activationCode === 'Y');
      }
    });
  ...
  advSearch() {
    this.documentrkapService.getByData(this.search).subscribe(resp => {
      this.documentrkapService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  onDeleteConf() {
    this.documentrkapService.getById(this.target).subscribe(role => {
      this.documentrkapService.delete(role.d).subscribe(resp => {
        // this.getRoleData();
        notify({
          closeOnClick: true,
          displayTime: 3000,
