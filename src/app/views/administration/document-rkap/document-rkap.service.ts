...
import { DocumentRkap, KategoriAktif } from './document-rkap.model';
...
//   providedIn: 'root'
// })
// export class DocumentRkapService {

//   constructor() { }
// }
@Injectable()
export class DocumentRkapService {
  private resourceUrlRole = this.a.SERVER_URL + '/system/DocumentRKAP';
  ...
  getById(id: any): Observable<any> {
    ...
    return this.http.get(this.resourceUrlRole + '/retrieve?token=' + token + '&docid=' + id)
  }
  ...
  save(data: DocumentRkap): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
  ...
  update(data: DocumentRkap): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
    const today = new Date().toISOString().slice(0, 10);
    console.log(data);
  ...
  delete(data: any): Observable<any> {
    ...
    const token = localStorage.getItem('token');
    return this.http.get(this.resourceUrlRole + '/delete?token=' + token + '&docid=' + data.docid)
  }
