...
import { IndikatorOutput, KategoriAktif } from './indikator-output.model';
...
//   providedIn: 'root'
// })
// export class IndikatorOutputService {

//   constructor() { }
// }
@Injectable()
export class IndikatorOutputService {
  private resourceUrlRole = this.a.SERVER_URL + '/system/IndikatorOutput';
  ...
  getById(id: any): Observable<any> {
    ...
    return this.http.get(this.resourceUrlRole + '/retrieve?token=' + token + '&outid=' + id)
  }
  ...
  save(data: IndikatorOutput): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
  ...
  update(data: IndikatorOutput): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
    const today = new Date().toISOString().slice(0, 10);
    console.log(data);
  ...
  delete(data: any): Observable<any> {
    ...
    const token = localStorage.getItem('token');
    return this.http.get(this.resourceUrlRole + '/delete?token=' + token + '&outid=' + data.outid)
  }
