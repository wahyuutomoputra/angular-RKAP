...
import {Search} from './indikator-output.model';
...
@Component({
  selector: 'app-indikator-output',
  templateUrl: './indikator-output.component.html',
  styleUrls: ['./indikator-output.component.scss'],
  providers: [IndikatorOutputService]
})
// export class IndikatorOutputComponent implements OnInit {
...
// }
export class IndikatorOutputComponent implements AfterViewInit {
  ...
  constructor(
    ...
    @Inject(IndikatorOutputService) private indikatoroutputService: IndikatorOutputService
  ) {
    this.search = {
      outid: null,
      indikator: '',
    ...
    //this.indikatoroutputService.getAll()
    //.subscribe(resp => {
    //  console.log(resp);
  ...
  pagination(){
    ...
    this.indikatoroutputService.getLimit(this.offset,this.limitVal)
    .subscribe(resp => {
      console.log(resp);
      this.gridDataSource = resp.d.list;
  ...
  refresh() {
    this.indikatoroutputService.getAll().subscribe(resp => {
      this.gridDataSource = resp.d.list;
    }, err => {
      console.log(err);
    })
  ...
  getRoleData() {
    ...
    this.indikatoroutputService.getAll().subscribe(resp => {
      this.indikatoroutputService.getAllRoleAuth().subscribe(respAuth => {
  ...
  addNewButton() {
    ...
    const $customButton = $('<div id="addNewRole">').dxButton({
      icon: 'add',
      text: 'Tambah Indikator Output Baru',
      onClick: function () {
        ini.isAdd = true;
  ...
  searching() {
    this.indikatoroutputService.getByName(this.search.indikator).subscribe(resp => {
      this.indikatoroutputService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
      ...
      if (this.search.indikator !== '') {
        this.gridDataSource = resp.filter(role => role.roleName === this.search.indikator).filter(data => data.activationCode === 'Y');
      } else {
        this.gridDataSource = resp.filter(data => data.activationCode === 'Y');
      }
    });
  ...
  advSearch() {
    this.indikatoroutputService.getByData(this.search).subscribe(resp => {
      this.indikatoroutputService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  onDeleteConf() {
    this.indikatoroutputService.getById(this.target).subscribe(role => {
      this.indikatoroutputService.delete(role.d).subscribe(resp => {
        // this.getRoleData();
        notify({
          closeOnClick: true,
          displayTime: 3000,
