...
import {Menu, RiskTaksonomi, KategoriAktif} from '../risk-taksonomi.model';
...
@Component({
  selector: 'app-add-risk-taksonomi',
  templateUrl: './add-risk-taksonomi.component.html',
  providers: []
})
// export class AddRiskTaksonomiComponent implements OnInit {
...
// }
export class AddRiskTaksonomiComponent implements OnInit, AfterViewInit, AfterContentInit {
  @Input() isEdit;
  @Input() isDetail;
  @Input() editItem;
  @Input() addVisible;
  ...
  constructor(private risktaksonomiService: RiskTaksonomiService) {
    this.simpleProducts = risktaksonomiService.getSimpleProducts();
  ...
  ngOnInit() {
   if (this.isEdit || this.isDetail) {
     ...
     this.risktaksonomiService.getById(this.editItem).subscribe(respRole => {
       console.log(respRole);
       console.log(this.editItem);
       const nilai: string = respRole.d.taxoid;
       this.role = {
         taxoid: respRole.d.taxoid,
         gid: respRole.d.gid,
         //urutan: parseInt(nilai),
         taksonomi: respRole.d.taksonomi,
       };
 ...
 checkBoxToggled(e) {
  ...
  onSaveConf() {
    ...
    const nilai: number = this.role.taxoid;
    this.role.taxoid = nilai;
    let success = false;
    if (!this.isEdit) {
      this.role.taxoid = null;
      this.risktaksonomiService.save(this.role).subscribe(resp => {
        console.log(resp);
        success = true;
        this.menuTree.forEach(menuItem => {
          if (menuItem.read || menuItem.write) {
            ...
            this.risktaksonomiService.saveRoleAuth({
              read: read,
              write: write,
        ...
        if (success) {
          ...
          notify({message: 'Risk Taksonomi berhasil disimpan', position: {my: 'center top', at: 'center top'}},
           'success', 3000);
          this.hide();
        } else {
      ...
      this.risktaksonomiService.update(this.role).subscribe(resp => {
       ...
       success = true;
        ...
        this.menuTree.forEach(menuItem => {
          if (menuItem.read || menuItem.write) {
            ...
            if (typeof menuItem.authId === 'undefined') {
             ...
             this.risktaksonomiService.updateRoleAuth({
            ...
            if (typeof menuItem.authId !== 'undefined') {
              this.risktaksonomiService.deleteRoleAuth(menuItem.authId).subscribe(() => { }, () => {
                success = false;
              });
            }
          }
        ...
        if (success) {
          this.options.message = 'Risk Taksonomi updated';
          notify(this.options, 'success', 3000);
          this.hide();
        } else {
