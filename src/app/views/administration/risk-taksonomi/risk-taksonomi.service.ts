...
import { RiskTaksonomi, KategoriAktif } from './risk-taksonomi.model';
...
//   providedIn: 'root'
// })
// export class RiskTaksonomiService {

//   constructor() { }
// }
@Injectable()
export class RiskTaksonomiService {
  private resourceUrlRole = this.a.SERVER_URL + '/system/RiskTaksonomi';
  ...
  getById(id: any): Observable<any> {
    ...
    return this.http.get(this.resourceUrlRole + '/retrieve?token=' + token + '&taxoid=' + id)
  }
  ...
  save(data: RiskTaksonomi): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
  ...
  update(data: RiskTaksonomi): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
    const today = new Date().toISOString().slice(0, 10);
    console.log(data);
  ...
  delete(data: any): Observable<any> {
    ...
    const token = localStorage.getItem('token');
    return this.http.get(this.resourceUrlRole + '/delete?token=' + token + '&taxoid=' + data.taxoid)
  }
