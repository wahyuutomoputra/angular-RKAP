...
import {Search} from './risk-taksonomi.model';
...
@Component({
  selector: 'app-risk-taksonomi',
  templateUrl: './risk-taksonomi.component.html',
  styleUrls: ['./risk-taksonomi.component.scss'],
  providers: [RiskTaksonomiService]
})
// export class RiskTaksonomiComponent implements OnInit {
...
// }
export class RiskTaksonomiComponent implements AfterViewInit {
  ...
  constructor(
    ...
    @Inject(RiskTaksonomiService) private risktaksonomiService: RiskTaksonomiService
  ) {
    this.search = {
      taxoid: '',
      gid: '',
    ...
    //this.risktaksonomiService.getAll()
    //.subscribe(resp => {
    //  console.log(resp);
  ...
  pagination(){
    ...
    this.risktaksonomiService.getLimit(this.offset,this.limitVal)
    .subscribe(resp => {
      console.log(resp);
      this.gridDataSource = resp.d.list;
  ...
  refresh() {
    this.risktaksonomiService.getAll().subscribe(resp => {
      this.gridDataSource = resp.d.list;
    }, err => {
      console.log(err);
    })
  ...
  getRoleData() {
    ...
    this.risktaksonomiService.getAll().subscribe(resp => {
      this.risktaksonomiService.getAllRoleAuth().subscribe(respAuth => {
  ...
  addNewButton() {
    ...
    const $customButton = $('<div id="addNewRole">').dxButton({
      icon: 'add',
      text: 'Tambah Risk Taksonomi Baru',
      onClick: function () {
        ini.isAdd = true;
  ...
  searching() {
    this.risktaksonomiService.getByName(this.search.taksonomi).subscribe(resp => {
      this.risktaksonomiService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
      ...
      if (this.search.taksonomi !== '') {
        this.gridDataSource = resp.filter(role => role.roleName === this.search.taksonomi).filter(data => data.activationCode === 'Y');
      } else {
        this.gridDataSource = resp.filter(data => data.activationCode === 'Y');
      }
    });
  ...
  advSearch() {
    this.risktaksonomiService.getByData(this.search).subscribe(resp => {
      this.risktaksonomiService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  onDeleteConf() {
    this.risktaksonomiService.getById(this.target).subscribe(role => {
      this.risktaksonomiService.delete(role.d).subscribe(resp => {
        // this.getRoleData();
        notify({
          closeOnClick: true,
          displayTime: 3000,
