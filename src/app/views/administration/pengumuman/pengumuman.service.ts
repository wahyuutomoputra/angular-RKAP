...
import { Pengumuman, KategoriAktif } from './pengumuman.model';
...
@Injectable()
export class PengumumanService {
  private resourceUrlRole = this.a.SERVER_URL + '/system/Pengumuman';
  ...
  getById(id: any): Observable<any> {
    ...
    return this.http.get(this.resourceUrlRole + '/retrieve?token=' + token + '&pid=' + id)
  }
  ...
  save(data: Pengumuman): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
  ...
  update(data: Pengumuman): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
    const today = new Date().toISOString().slice(0, 10);
    console.log(data);
  ...
  delete(data: any): Observable<any> {
    ...
    const token = localStorage.getItem('token');
    return this.http.get(this.resourceUrlRole + '/delete?token=' + token + '&pid=' + data.pid)
  }
